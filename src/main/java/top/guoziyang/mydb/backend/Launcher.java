...
public class Launcher {
    ...
    private static void openDB(String path, long mem) {
        TransactionManager tm = TransactionManager.open(path);//事务
        DataManager dm = DataManager.open(path, mem, tm);//数据
        VersionManager vm = new VersionManagerImpl(tm, dm);//版本
        TableManager tbm = TableManager.open(path, vm, dm);//表
        new Server(port, tbm).start();//服务
    }
